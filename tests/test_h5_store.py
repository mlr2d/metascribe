from metascribe.h5_store import H5Store
import pandas as pd
import logging
...
def test_write_df():
    h5_path = "tests/files/test_store.h5"
    with H5Store(h5_path, mode='w', lock=True) as store:
        logging.info(f"Opened HDF5 store at {h5_path}")
        store.put_df("/activity_log/x/y", df1)
        

def test_read_df():
    h5_path = "tests/files/test_store.h5"
    with H5Store(h5_path, mode='r', lock=True) as store:
        logging.info(f"Opened HDF5 store at {h5_path}")
...
def test_write_json():
    h5_path = "tests/files/test_store.h5"
    with H5Store(h5_path, mode='a', lock=True) as store:
        logging.info(f"Opened HDF5 store at {h5_path}")
